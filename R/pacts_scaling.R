...
pacts_scaling <- function(data, id, date, menses, ovtoday, lower_cyclength_bound = 21, upper_cyclength_bound = 35,
  ...
  `%>%` <- magrittr::`%>%`
  if (!missing(next_menses_max_window)) {
    warning("next_menses_max_window no longer affects the result (as of 0.1.7) -- ",
            "a real closing menses now prevents imputation at any distance, not just ",
            "within this many days. See ?pacts_scaling.", call. = FALSE)
  }
  id <- rlang::enquo(id)
