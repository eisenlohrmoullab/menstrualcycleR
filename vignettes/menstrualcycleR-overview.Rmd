...
**These bounds do not decide which cycles get scaled.** When a cycle has a *confirmed* ovulation (`ovtoday == 1`), its cycle length is never consulted: the cycle is scaled if its **phase** lengths fit, i.e. luteal (ovulation to next menses) within `luteal_phase_min_days`-`luteal_phase_max_days` (7-18) and follicular (menses to ovulation) within `follicular_phase_min_days`-`follicular_phase_max_days` (8-25). A confirmed 45-day cycle whose phases both fit will be scaled at the default bounds. This is deliberate -- scaling on cycle length alone would discard a valid phase that is bookended by real anchors -- but it means the cycle-length bounds are **not** an inclusion criterion.

**If you want cycles outside a length range excluded, filter after scaling.** `mcyclength` and `cycle_incomplete` are both returned:

```r
scaled |> dplyr::filter(cycle_incomplete == 0, mcyclength >= 21, mcyclength <= 35)
```

Note that on `cycle_incomplete == 1` rows `mcyclength` is days-observed-so-far, not a cycle length.
