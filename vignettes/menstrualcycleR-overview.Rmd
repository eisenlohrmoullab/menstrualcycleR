...
```{r cycle_plot_menses_id_2_raw_plot, fig.width=7, fig.height=6}
...
- This model includes:
  ...
  - A **random smooth** capturing **individual-specific trajectories** across the cycle.
...
- **`s(scaled_cycleday_impute)`**:  
  ...
  - The **very small p-value** suggests this population-level pattern is **strongly significant** 
...
> These results suggest that on average, `symptom` varies in a nonlinear way, significantly across the cycle and that individuals also show **unique, person-specific patterns** of `symptom` change over time -- i.e. there are significant individual diffeerences in nonlinear trends across the cycle.  
...
```{r gam_plot, fig.width=8, fig.height=6}
# Plotting
...
```{r total_within_person, fig.width=8, fig.height=6}
...
```{r total_within_person_cycle, fig.width=8, fig.height=6}
# Step 1: Create the data frame
variance_df <- data.frame(
  Component = c("Fixed effect of the cycle", 
                "Random slope of the cycle"),
